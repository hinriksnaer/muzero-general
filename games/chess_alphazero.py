...
class Game(AbstractGame):
    ...
    def human_to_action(self):
        ...
        while True:
            try:
                choice = input(
                            f"Enter the row (1, 2 or 3) to play: "
                        )
    ...
    def action_to_string(self, action_number):
        ...
        """
        return self.env._env.decode(action_number)
...
class Chess:
    ...
    def render(self):
        flipped_unicode = self.board.unicode()
        unicode = ''
        for char in flipped_unicode:
            if char in _RENDER_SWAP.keys():
                unicode += _RENDER_SWAP[char]
            else:
                unicode += char
        print(unicode)
